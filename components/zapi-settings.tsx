...
export default function ZApiSettings(){
 ...
 <div className="wa-test-footer"><Button disabled={busy} type="submit">Guardar credenciais</Button><Button type="button" variant="outline" disabled={busy||!configured} onClick={async()=>{setBusy(true);setError('');setNote('');try{const r=await api<{ok:boolean;connected?:boolean;error?:string}>('/admin/whatsapp/zapi/test','POST');if(!r.ok)throw new Error(r.error??'Não foi possível testar o acesso à Z-API.');setNote(r.connected?'Acesso confirmado. A instância já tem WhatsApp associado.':'Acesso confirmado. A instância aguarda associação.');}catch(e){setError((e as Error).message);}finally{setBusy(false);}}}>Testar acesso</Button></div>
