...
### Edição das regras públicas
Configurações → Regras permite editar título, introdução e secções, incluindo os exemplos de pontuação. Guardar e publicar regras atualiza imediatamente /regras na instalação em uso. O texto é guardado na base de dados, preservado em atualizações e protegido contra gravações concorrentes desatualizadas. A edição é apenas editorial: não altera o motor de pontuação, o sorteio ou as movimentações automáticas. A página pública não exige autenticação e apresenta erro explícito se não conseguir carregar o conteúdo. Teste isolado: node tests/public-rules-api.mjs.

### IA de consulta no grupo
Configurações → Geral → IA no grupo WhatsApp permite ativar/desativar, simular uma pergunta como jogador e consultar as últimas respostas preparadas. A integração utiliza a chave cifrada existente e gpt-4.1-mini na Responses API, com saída estruturada e store:false. A OpenAI classifica apenas a pergunta e recebe os títulos públicos das regras; os horários, jogadores e pontos são consultados localmente e apresentados por respostas determinísticas. Não altera dados do torneio.
Ativada por defeito após instalar esta versão, requer chave válida, WhatsApp ligado e jogador ativo/validado no grupo selecionado. Ignora mensagens do próprio número do bot. Conversas irrelevantes devem ficar em silêncio. Suporta próprios jogos hoje, amanhã, esta semana ou próxima ronda; próprios pontos; classificação atual por divisão; secções das regras publicadas; data prevista de movimentação e ajuda de utilização. Outras datas, terceiros específicos e pedidos de alteração recebem esclarecimento. Ausências e voluntários mantêm o fluxo anterior separado.
Limites persistentes: 4 interpretações por jogador/minuto, 60 por hora na instalação (inclui simulador e mensagens classificadas como irrelevantes). Respostas ficam na fila de mensagens com validade de 5 minutos; desativar a IA ou mudar de grupo cancela respostas pendentes ao processá-las. Duplicados são evitados por identificador da mensagem. Falhas da API aparecem no backoffice; não são despejadas no grupo. O histórico mostra respostas preparadas; o estado de envio está em WhatsApp → Mensagens.
Validação: npm test --prefix server e node tests/ai-bot-api.mjs. A prova isolada usa API simulada e dados temporários; um teste adicional real com a chave existente classificou “Tenho algum jogo hoje?” como games/today, sem enviar mensagens WhatsApp.
