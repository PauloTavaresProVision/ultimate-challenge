...
export type PlayerStandingMonth={month:string;division:string;archived:boolean;rows:StandingRow[]};
...
export function playerStandings(playerId:string,players:Player[],games:Game[],month:string,archives:{month:string;table:ArchivedRow[]}[]=[]):PlayerStandingMonth[]{
  ...
  return [{month,division:me.division,archived:false,rows:project(current,me.division)},...archives.filter(a=>a.month<month).sort((a,b)=>b.month.localeCompare(a.month)).map(a=>{
    const division=a.table.find(p=>p.id===playerId)?.division??me.division;
    return {month:a.month,division,archived:true,rows:project(a.table,division)};
  })];
}
