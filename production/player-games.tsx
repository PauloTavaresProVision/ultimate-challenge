...
export type PlayerGamesData = { playerId: string; standings: PlayerStandingMonth[]; people: { id: string; name: string; side: string }[]; games: { id: string; absentIds?: string[]; duration?: number; division: string; date: string; time: string; court: { name: string; location: string }; a: string[]; b: string[]; winner: string | null }[] };
export function PlayerGames({ data, refresh }: { data: PlayerGamesData | null; refresh: () => Promise<void> }) {
  ...
  return <div className="player-games-workspace">
    ...
    <div className="player-area-tabs" role="group" aria-label="Área do jogador"><Button variant={view==='games'?'default':'ghost'} aria-pressed={view==='games'} onClick={()=>setView('games')}>Os meus jogos</Button><Button variant={view==='standings'?'default':'ghost'} aria-pressed={view==='standings'} onClick={()=>{setView('standings');void refresh().catch(e=>setError(e.message));}}>Classificação</Button></div>
    {view==='games'&&<div className="player-games-tabs" role="group" aria-label="Jogos a mostrar">{[['mine', 'Só os meus'], ['all', 'Todos os jogos']].map(([value,label]) => <Button key={value} variant={filter === value ? 'default' : 'ghost'} aria-pressed={filter === value} onClick={() => setFilter(value)}>{label}</Button>)}</div>}
    {notice && <p className="wa-success" role="status"><Check size={17} />{notice}</p>}
    {error && !pending && <p className="form-error" role="alert">{error}</p>}
    {view==='standings'?<PlayerStandings months={data?.standings??[]} onGames={()=>{setFilter('mine');setView('games');}}/>: !data ? <section className="panel"><h2>Não foi possível carregar os jogos</h2><p>Usa Atualizar para tentar novamente.</p></section> : !games.length ? <section className="player-games-empty"><CalendarDays size={30} /><h2>{filter === 'mine' ? 'Ainda não tens jogos publicados' : 'A próxima ronda está a ser preparada'}</h2><p>Os encontros ficam disponíveis aqui quando a organização os publicar.</p></section> : <div className="player-match-grid">{games.map(g => <article className="player-match" key={g.id}>
      <header><span className="badge">{g.division}</span><span className={g.winner ? 'player-match-done' : ''}>{g.absentIds?.length ? 'Aguarda suplente' : g.winner ? 'Concluído' : 'Por jogar'}</span></header>
      ...
      <div className="player-match-teams">{(['a','b'] as const).map(team => <div key={team} className={`player-match-team ${g.winner === team ? 'is-winner' : ''}`}><span className="player-team-label">Dupla {team.toUpperCase()}{g.winner === team && <Trophy size={15} />}</span>{g[team].map(id => <div key={id}><strong>{g.absentIds?.includes(id) ? 'Aguarda suplente' : name(id)}{id === data.playerId && <small>Tu</small>}</strong><span>{data.people.find(p => p.id === id)?.side}</span></div>)}</div>)}</div>
      {g.winner ? <p className="player-match-result"><Check size={15} />Vitória: {g[g.winner as 'a'|'b'].map(name).join(' / ')}</p> : mine(g) ? <footer><Button disabled={busy || !!g.absentIds?.length || g.date > new Date().toISOString().slice(0,10)} onClick={() => { setError(''); setPending({id:g.id,outcome:'win'}); }}>Ganhámos</Button><Button variant="outline" disabled={busy || g.date > new Date().toISOString().slice(0,10)} onClick={() => { setError(''); setPending({id:g.id,outcome:'loss'}); }}>Perdemos</Button><small>{g.date > new Date().toISOString().slice(0,10) ? 'Disponível no dia do jogo' : 'Regista apenas depois de terminar o jogo'}</small></footer> : <p className="player-match-result">O resultado será registado pelos participantes.</p>}
    </article>)}</div>}
    <Dialog open={!!pending} onOpenChange={open => { if (!open && !busy) { setPending(null); setError(''); } }}><DialogContent showCloseButton={!busy}><DialogHeader><DialogTitle>Confirmar resultado</DialogTitle><DialogDescription>Venceu a dupla {winningTeam.map(name).join(' / ')}. O resultado contará para a classificação. Depois de guardar, só a organização poderá corrigi-lo.</DialogDescription></DialogHeader>{error && <p className="form-error" role="alert">{error}</p>}<DialogFooter><Button variant="outline" disabled={busy} onClick={() => { setPending(null); setError(''); }}>Voltar</Button><Button disabled={busy} onClick={async () => {
      ...
      try { await api(`/games/${encodeURIComponent(pending.id)}/result`, 'POST', { outcome: pending.outcome }); setPending(null); setNotice('Resultado guardado. Consulta a tua pontuação em Classificação.'); try { await refresh(); } catch { setError('O resultado foi guardado, mas a lista não atualizou. Usa Atualizar.'); } }
