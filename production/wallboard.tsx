...
function WallboardContent(){
  ...
  return <main className="wallboard">
    ...
    <div className="wall-summary"><span>{data?.games.length??0} jogos no dia · {new Set(data?.games.map(g=>g.court)).size} campos · {new Set(data?.games.map(g=>g.division)).size} {new Set(data?.games.map(g=>g.division)).size===1?'nível':'níveis'}</span><span>{next?`Próximo horário ${next}`:'Último horário do dia'}</span></div>
    {error&&<p className="wall-error" role="alert">{error} {data?'A mostrar a última atualização.':''}</p>}
    <div className={"wall-levels "+(levels.length===1?"wall-single":"")} style={{'--levels':Math.max(1,levels.length)} as React.CSSProperties}>
      ...
      {levels.map(level=><section className="wall-level" data-division={level} key={level}><div className="wall-level-label"><strong>{level}</strong><span>{slot}</span></div><div className="wall-courts" style={{'--courts':games.filter(g=>g.division===level).length} as React.CSSProperties}>{games.filter(g=>g.division===level).map(g=>{
