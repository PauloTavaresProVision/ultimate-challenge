...
try {
  if (!['M1+', 'M1', 'M2+', 'M2'].includes(division)) throw Error('Indica uma divisão válida em ROUND_DIVISION.');
  await db.$transaction(async tx => {
    ...
    }).filter(({text}) => typeof text === 'string' && (text.split(/\r?\n/).includes(division) || text.includes(`*Ultimate Challenge · ${division}*`)));
    if (candidates.length !== 1) throw Error(candidates.length
      ? `Existe mais de uma convocatória ${division} pendente. Nada foi enviado.`
      : `Não há convocatória ${division} pendente sem tentativas. Se ainda está em rascunho, publica o sorteio ${division} primeiro.`);
    ...
    if (message?.format === 'mentioned-reply-v1') {
      ...
      if (headers?.length !== 1 || headers[0] !== `*Ultimate Challenge · ${division}*` || !Array.isArray(message.mentions) || !message.mentions.length)
        throw Error(`A convocatória não contém apenas ${division} com menções. Nada foi alterado.`);
    } else {
      const players = await tx.player.findMany();
      const courts = await tx.court.findMany();
      const games = await tx.game.findMany({where:{published:true,division},orderBy:[{date:'asc'},{time:'asc'}]});
      let remaining = body;
      ...
      remaining = remaining.replace('🎾 Escada · Jogos','').split(/\r?\n/).filter(line=>line!==division).join('\n').replace(`Jogos: ${config.APP_ORIGIN}/jogos`,'').trim();
      if (remaining || !selected.length) throw Error(`Os jogos ${division} já não correspondem à mensagem guardada. Nada foi alterado.`);
    ...
    console.log(`\n${division} preparado para envio imediato.`);
  });
