...
export function installAI(app:Express,auth:RequestHandler,admin:RequestHandler){
 ...
 app.put('/api/admin/bot',auth,admin,async(req,res)=>{const {enabled}=z.object({enabled:z.boolean()}).parse(req.body);await db.$transaction(async tx=>{await tx.setting.upsert({where:{key:'bot-enabled'},create:{key:'bot-enabled',value:String(enabled)},update:{value:String(enabled)}});if(!enabled)await tx.outbox.updateMany({where:{kind:{in:['ai','journey_reply']},status:'pending'},data:{status:'cancelled',encryptedBody:''}});});res.json({enabled});});
