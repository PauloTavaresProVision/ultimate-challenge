...
const players:Player[]=['M1+','M1','M2'].flatMap((division,d)=>Array.from({length:4},(_,i)=>({id:`p${d}${i}`,name:`Player ${d}${i}`,phone:`+244900000${d}${i}`,birth:`${1980+i}-01-01`,side:i%2?'Direita':'Esquerda',division:division as Player['division'],status:'Ativo',verified:true,note:''})));
const games:Game[]=['M1+','M1','M2'].map((division,d)=>({id:`g${d}`,division:division as Game['division'],a:[`p${d}0`,`p${d}1`],b:[`p${d}2`,`p${d}3`],court:'c',round:1,date:'2026-09-01',time:'18:00',duration:90,winner:'a',published:true}));
test('Biweekly exchanges use one best and worst per side, simultaneously, within division limits',()=>{
 const moves=movementPlan(players,games,'2026-09-15');assert.equal(moves.length,8);assert.equal(new Set(moves.map(m=>m.id)).size,8);
...
test('Monthly archives have three champions, older wins ties, and the next month starts at zero',()=>{
 const record=monthRecord(players,games,'2026-09');assert.equal(record.champions.length,3);assert.equal(record.champions[0].player?.id,'p00');
